...
- entity、block entity、player data、mod NBT 内の owner、trusted players、
  anger/love cause、conversion player、projectile owner/thrower、leash data、
  villager gossip、BossBar players、raid saved data、scoreboard UUID values、
  player head profile、1.21 item component profile、Brain memories、
  Touhou Little Maid 形式の `owner_uuid`。
- singleplayer transfer：mapping が 1 件だけなら `level.dat` の `Data.Player` を
  `playerdata/<targetUuid>.dat` に自動コピーします。mapping が複数ある場合は
  `--singleplayer-name <name>` を使います。
